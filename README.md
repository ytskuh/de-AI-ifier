...
uv run deaiify fix <article> [--write]          # mechanical fixes (swap/delete/split), diff preview
uv run deaiify check <article> --profile <name> # pass/fail vs baseline percentile bands
uv run deaiify stat <article> [--pair NAME|all] [--classes] [--top N]
...
Kobak excess words / slop-forensics / Nous ANTI-SLOP) and document heuristics
...
⚠ License debt (deliberate, pre-distribution): Nous-derived rules
(`rulepacks/en/Deaiify/Nous*.yml`, unlicensed upstream) — excisable by deleting the
files; see the design doc's debt list.
