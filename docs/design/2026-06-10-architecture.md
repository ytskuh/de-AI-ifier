...
|---|---|---|---|
| Lexical | Vale subprocess: `ai-tells` + `Deslop` packages + generated `Deaiify` style (Kobak excess words, slop-forensics lists; optionally Nous tables, opt-in) | per-span findings with severity | human rewrite; mechanical fixes exist but are **banned** (below) |
...
1. Rules parsed from Nous ANTI-SLOP.md — upstream has no license. RESOLVED in the
   distributed repo: not generated by default, gitignored (`rulepacks/en/Deaiify/Nous*.yml`),
   and excluded from history. Users who want them opt in locally with
   `build_rulepacks.py --include-nous`, which fetches Nous ANTI-SLOP.md and writes
   the (gitignored) files into their own working tree — the all-rights-reserved
   content is then used privately, never redistributed by this repo.
